...
def likelihood(
    self, depth=0, which_image=0,
    speaker_rationality=1.0, speaker=0, listener_rationality=1.0,
    ...
    self.image_priors = np.log(
        np.ones(
                (max_sentence_length + 1, self.images.shape[0])
...
def ana_beam(rsa, initial_world_prior, speaker_rationality, target, speaker, pass_prior=True, listener_rationality=1.0, depth=0, start_from=[], beam_width=len(sym_set), cut_rate=1, decay_rate=0.0, beam_decay=0,):
    ...
    # default: start from entry list of tokens
    state = RSA_State(initial_world_prior,
                      listener_rationality=listener_rationality)
    context_sentence = start_from
    state.context_sentence = context_sentence
